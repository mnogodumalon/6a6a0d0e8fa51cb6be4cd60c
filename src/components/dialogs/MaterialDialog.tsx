...
export type MaterialDialogDefaults = Omit<Material['fields'], 'kategorie' | 'einheit'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kategorie: LOOKUP_OPTIONS['material']?.['kategorie'] ?? [],
...
export function MaterialDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MaterialDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "bezeichnung": string | null, // Bezeichnung\n  "artikelnummer": string | null, // Artikelnummer\n  "kategorie": LookupValue | null, // Kategorie (select one key: "rohre_leitungen" | "befestigungsmaterial" | "elektromaterial" | "daemmmaterial" | "werkzeug" | "sanitaer" | "heizung" | "sonstiges") mapping: rohre_leitungen=Rohre & Leitungen, befestigungsmaterial=Befestigungsmaterial, elektromaterial=Elektromaterial, daemmmaterial=Dämmmaterial, werkzeug=Werkzeug, sanitaer=Sanitär, heizung=Heizung, sonstiges=Sonstiges\n  "einheit": LookupValue | null, // Einheit (select one key: "kubikmeter" | "pauschal" | "quadratmeter" | "stueck" | "meter" | "kilogramm" | "liter") mapping: kubikmeter=Kubikmeter, pauschal=Pauschal, quadratmeter=Quadratmeter, stueck=Stück, meter=Meter, kilogramm=Kilogramm, liter=Liter\n  "einkaufspreis": number | null, // Einkaufspreis (€)\n  "verkaufspreis": number | null, // Verkaufspreis (€)\n  "bemerkungen_material": string | null, // Bemerkungen\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    'bezeichnung': (
      <div key="bezeichnung" className="space-y-1.5">
        ...
        <Input
          id="bezeichnung"
          placeholder="z. B. Kupferrohr DN15"
      ...
      <div key="kategorie" className="space-y-1.5">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="rohre_leitungen">Rohre & Leitungen</SelectItem>
            <SelectItem value="befestigungsmaterial">Befestigungsmaterial</SelectItem>
            <SelectItem value="elektromaterial">Elektromaterial</SelectItem>
            <SelectItem value="daemmmaterial">Dämmmaterial</SelectItem>
    ...
    'bemerkungen_material': (
      <div key="bemerkungen_material" className="space-y-1.5">
        <Label htmlFor="bemerkungen_material">Bemerkungen</Label>
        <Textarea
          id="bemerkungen_material"
          placeholder="Hersteller, Verfügbarkeit, Lagertipps..."
          value={fields.bemerkungen_material ?? ''}
          onChange={e => setFields(f => ({ ...f, bemerkungen_material: e.target.value }))}
          rows={3}
        />
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"bezeichnung": "Bezeichnung", "artikelnummer": "Artikelnummer", "kategorie": "Kategorie", "einheit": "Einheit", "einkaufspreis": "Einkaufspreis (€)", "verkaufspreis": "Verkaufspreis (€)", "bemerkungen_material": "Bemerkungen"};
