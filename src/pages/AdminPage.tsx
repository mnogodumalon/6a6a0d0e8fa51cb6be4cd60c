...
import { PositionenViewDialog } from '@/components/dialogs/PositionenViewDialog';
import { PruefprotokolleDialog } from '@/components/dialogs/PruefprotokolleDialog';
import { PruefprotokolleViewDialog } from '@/components/dialogs/PruefprotokolleViewDialog';
...
const KUNDEN_FIELDS = [
  ...
  { key: 'bemerkungen_kunde', label: 'Bemerkungen', type: 'string/textarea' },
];
const MATERIAL_FIELDS = [
  { key: 'bezeichnung', label: 'Bezeichnung', type: 'string/text' },
  { key: 'artikelnummer', label: 'Artikelnummer', type: 'string/text' },
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'rohre_leitungen', label: 'Rohre & Leitungen' }, { key: 'befestigungsmaterial', label: 'Befestigungsmaterial' }, { key: 'elektromaterial', label: 'Elektromaterial' }, { key: 'daemmmaterial', label: 'Dämmmaterial' }, { key: 'werkzeug', label: 'Werkzeug' }, { key: 'sanitaer', label: 'Sanitär' }, { key: 'heizung', label: 'Heizung' }, { key: 'sonstiges', label: 'Sonstiges' }] },
  { key: 'einheit', label: 'Einheit', type: 'lookup/select', options: [{ key: 'kubikmeter', label: 'Kubikmeter' }, { key: 'pauschal', label: 'Pauschal' }, { key: 'quadratmeter', label: 'Quadratmeter' }, { key: 'stueck', label: 'Stück' }, { key: 'meter', label: 'Meter' }, { key: 'kilogramm', label: 'Kilogramm' }, { key: 'liter', label: 'Liter' }] },
  { key: 'einkaufspreis', label: 'Einkaufspreis (€)', type: 'number' },
  { key: 'verkaufspreis', label: 'Verkaufspreis (€)', type: 'number' },
  { key: 'bemerkungen_material', label: 'Bemerkungen', type: 'string/textarea' },
];
const AUFTRAEGE_FIELDS = [
  { key: 'auftragsnummer', label: 'Auftragsnummer', type: 'string/text' },
  { key: 'bezeichnung', label: 'Auftragsbezeichnung', type: 'string/text' },
  { key: 'kunde', label: 'Kunde', type: 'applookup/select', targetEntity: 'kunden', targetAppId: 'KUNDEN', displayField: 'vorname' },
  { key: 'ansprechpartner', label: 'Ansprechpartner beim Kunden', type: 'string/text' },
  { key: 'auftragsdatum', label: 'Auftragsdatum', type: 'date/date' },
  { key: 'fertigstellungsdatum', label: 'Gewünschtes Fertigstellungsdatum', type: 'date/date' },
  ...
  { key: 'bemerkungen_auftrag', label: 'Bemerkungen', type: 'string/textarea' },
];
const POSITIONEN_FIELDS = [
  { key: 'auftrag', label: 'Auftrag', type: 'applookup/select', targetEntity: 'auftraege', targetAppId: 'AUFTRAEGE', displayField: 'auftragsnummer' },
  { key: 'positionstyp', label: 'Positionstyp', type: 'lookup/radio', options: [{ key: 'arbeitsleistung', label: 'Arbeitsleistung' }, { key: 'material_typ', label: 'Material' }, { key: 'pauschale', label: 'Pauschale' }, { key: 'sonstiges_typ', label: 'Sonstiges' }] },
  { key: 'positions_bezeichnung', label: 'Bezeichnung', type: 'string/text' },
  ...
  { key: 'einheit_position', label: 'Einheit', type: 'lookup/select', options: [{ key: 'stueck', label: 'Stück' }, { key: 'meter', label: 'Meter' }, { key: 'kilogramm', label: 'Kilogramm' }, { key: 'liter', label: 'Liter' }, { key: 'quadratmeter', label: 'Quadratmeter' }, { key: 'stunden', label: 'Stunden' }, { key: 'pauschal', label: 'Pauschal' }] },
  { key: 'einzelpreis', label: 'Einzelpreis (€)', type: 'number' },
  { key: 'gesamtpreis', label: 'Gesamtpreis (€)', type: 'number' },
  { key: 'bemerkungen_position', label: 'Bemerkungen', type: 'string/textarea' },
];
const PRUEFPROTOKOLLE_FIELDS = [
  ...
  { key: 'pruefdatum', label: 'Prüfdatum', type: 'date/date' },
  { key: 'pruefer_vorname', label: 'Prüfer Vorname', type: 'string/text' },
  { key: 'pruefer_nachname', label: 'Prüfer Nachname', type: 'string/text' },
  { key: 'pruefpunkte', label: 'Prüfpunkte', type: 'string/textarea' },
  { key: 'ergebnis', label: 'Prüfergebnis', type: 'lookup/radio', options: [{ key: 'bestanden', label: 'Bestanden' }, { key: 'nicht_bestanden', label: 'Nicht bestanden' }, { key: 'nachbesserung_erforderlich', label: 'Nachbesserung erforderlich' }] },
  { key: 'maengelbeschreibung', label: 'Mängelbeschreibung', type: 'string/textarea' },
  { key: 'fotos', label: 'Fotos / Dokumente', type: 'file' },
  { key: 'freigabe_erteilt', label: 'Freigabe erteilt', type: 'bool' },
...
const ENTITY_TABS = [
  ...
  { key: 'positionen', label: 'Positionen', pascal: 'Positionen' },
  { key: 'pruefprotokolle', label: 'Prüfprotokolle', pascal: 'Pruefprotokolle' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'positionen': return (data as any).positionen as Positionen[] ?? [];
      case 'pruefprotokolle': return (data as any).pruefprotokolle as Pruefprotokolle[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'positionen': return POSITIONEN_FIELDS;
      case 'pruefprotokolle': return PRUEFPROTOKOLLE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'positionen': return {
        create: (fields: any) => LivingAppsService.createPositionenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updatePositionenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deletePositionenEntry(id),
      };
      case 'pruefprotokolle': return {
        create: (fields: any) => LivingAppsService.createPruefprotokolleEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updatePruefprotokolleEntry(id, fields),
        remove: (id: string) => LivingAppsService.deletePruefprotokolleEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'positionen' || dialogState?.entity === 'positionen') && (
        <PositionenDialog
          open={createEntity === 'positionen' || dialogState?.entity === 'positionen'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'positionen' ? handleUpdate : (fields: any) => handleCreate('positionen', fields)}
          defaultValues={dialogState?.entity === 'positionen' ? dialogState.record?.fields : undefined}
      ...
      {(createEntity === 'pruefprotokolle' || dialogState?.entity === 'pruefprotokolle') && (
        <PruefprotokolleDialog
          open={createEntity === 'pruefprotokolle' || dialogState?.entity === 'pruefprotokolle'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'pruefprotokolle' ? handleUpdate : (fields: any) => handleCreate('pruefprotokolle', fields)}
          defaultValues={dialogState?.entity === 'pruefprotokolle' ? dialogState.record?.fields : undefined}
          auftraegeList={(data as any).auftraege ?? []}
      ...
      {viewState?.entity === 'positionen' && (
        <PositionenViewDialog
          open={viewState?.entity === 'positionen'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'positionen', record: r }); }}
          auftraegeList={(data as any).auftraege ?? []}
          materialList={(data as any).material ?? []}
        />
      )}
      {viewState?.entity === 'pruefprotokolle' && (
        <PruefprotokolleViewDialog
          open={viewState?.entity === 'pruefprotokolle'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'pruefprotokolle', record: r }); }}
          auftraegeList={(data as any).auftraege ?? []}
        />
      )}
