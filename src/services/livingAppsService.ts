...
import type { Kunden, Material, Auftraege, Positionen, Pruefprotokolle, CreateKunden, CreateMaterial, CreateAuftraege, CreatePositionen, CreatePruefprotokolle } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- POSITIONEN ---
  static async getPositionen(): Promise<Positionen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.POSITIONEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Positionen[];
    return enrichLookupFields(records, 'positionen');
  }
  static async getPositionenEntry(id: string): Promise<Positionen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.POSITIONEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Positionen;
    return enrichLookupFields([record], 'positionen')[0];
  }
  static async createPositionenEntry(fields: CreatePositionen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.POSITIONEN}/records`, { fields: cleanFieldsForApi(fields as any, 'positionen') });
    return { ...data, record_id: data.id };
  }
  static async updatePositionenEntry(id: string, fields: Partial<CreatePositionen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.POSITIONEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'positionen') });
    return { ...data, record_id: data.id };
  }
  static async deletePositionenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.POSITIONEN}/records/${id}`);
  }

  // --- PRUEFPROTOKOLLE ---
  static async getPruefprotokolle(): Promise<Pruefprotokolle[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.PRUEFPROTOKOLLE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Pruefprotokolle[];
    return enrichLookupFields(records, 'pruefprotokolle');
  }
  static async getPruefprotokolleEntry(id: string): Promise<Pruefprotokolle | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.PRUEFPROTOKOLLE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Pruefprotokolle;
    return enrichLookupFields([record], 'pruefprotokolle')[0];
  }
  static async createPruefprotokolleEntry(fields: CreatePruefprotokolle): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.PRUEFPROTOKOLLE}/records`, { fields: cleanFieldsForApi(fields as any, 'pruefprotokolle') });
    return { ...data, record_id: data.id };
  }
  static async updatePruefprotokolleEntry(id: string, fields: Partial<CreatePruefprotokolle>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.PRUEFPROTOKOLLE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'pruefprotokolle') });
    return { ...data, record_id: data.id };
  }
  static async deletePruefprotokolleEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.PRUEFPROTOKOLLE}/records/${id}`);
  }
